class Admin::DirectorController < ApplicationController
  ...
  def create
    ...
    if @director.save
      flash[:notice] = "Director #{@director.name} creado correctamente."
  ...
  def update
    ...
    if @director.update_attributes(director_params)
      flash[:notice] = "Director #{@director.name} actualizado correctamente."
  ...
  def destroy
    @director = Director.find(params[:id])
    @director.destroy
    flash[:notice] = "Director #{@director.name} eliminado correctamente."
    redirect_to :action => 'index'
  end
